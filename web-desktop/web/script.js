    ...
    try {
      ...
      if (error) {
        ...
        if (error.message && (error.message.includes('column') || error.message.includes('incomes') || error.message.includes('currency') || error.code === 'PGRST204')) {
          ...
          await supaClient.from('user_data').upsert(basicPayload, { onConflict: 'user_id' }).catch(() => {});
        } else {
          const { error: updateErr } = await supaClient.from('user_data').update({
            budget: budget,
            expenses: expenses,
            subscriptions: subscriptions,
            incomes: incomes,
...
function startSupabaseSync(userId) {
  ...
  const fetchPromise = supaClient.from('user_data').select('*').eq('user_id', userId).maybeSingle();
...
const CURRENT_APP_VERSION = 'v2.9.0';
