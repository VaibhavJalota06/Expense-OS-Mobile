...
function updateUI() {
  ...
  if (budget === 0) {
    if (statRemainingEl) statRemainingEl.textContent = isCardMasked('stat-remaining') ? '••••••' : formatCurrency(0, 'stat-remaining');
    if (statPercentEl) statPercentEl.textContent = 'Budget Not Set (Click to Set ✏️)';
  } else {
    if (statRemainingEl) statRemainingEl.textContent = formatCurrency(remaining, 'stat-remaining');
...
function markSubAsPaid(subId) {
  ...
  sub.last_paid_date = today;
  sub.lastPaidDate = today;

  const newExpense = {
    id: 'bill_pay_' + sub.id + '_' + Date.now().toString(36),
    source_id: String(sub.id),
    source_type: 'subscription',
    amount: Number(sub.amount),
    category: sub.category || 'Services & Subscriptions',
    description: `Bill Payment: ${sub.name}`,
    title: `Bill Payment: ${sub.name}`,
    payment: 'Auto-Pay',
...
async function deleteSubscription(subId) {
  ...
  if (ok) {
    const subToDelete = subscriptions.find(s => String(s.id) === String(subId));
    subscriptions = subscriptions.filter(s => String(s.id) !== String(subId));
    
    // Cascading deletion: remove all expense logs related to this subscription
    if (subToDelete) {
      const subName = (subToDelete.name || subToDelete.title || '').trim().toLowerCase();
      expenses = expenses.filter(e => {
        if (e.source_id && String(e.source_id) === String(subId)) return false;
        if (e.id && String(e.id).startsWith('bill_pay_')) return false;
        if (subName) {
          const desc = (e.description || e.title || '').trim().toLowerCase();
          if (desc === `bill payment: ${subName}` || desc === `${subName} (subscription)` || desc === `${subName} (recurring bill)`) {
            return false;
          }
        }
        return true;
...
async function deleteTransaction(id) {
  ...
  if (ok) {
    const itemToDelete = expenses.find(item => String(item.id) === String(id));
    expenses = expenses.filter(item => String(item.id) !== String(id));
    
    // If this was a recurring bill payment, reset the bill's paid status
    if (itemToDelete) {
      const desc = (itemToDelete.description || itemToDelete.title || '').trim();
      if (desc.toLowerCase().startsWith('bill payment: ')) {
        const billName = desc.substring(14).trim().toLowerCase();
        subscriptions.forEach(s => {
          const sName = (s.name || s.title || '').trim().toLowerCase();
          if (sName === billName) {
            s.lastPaidMonth = null;
            s.is_paid = false;
            s.isPaid = false;
            s.last_paid_date = null;
            s.lastPaidDate = null;
          }
        });
      }
...
const CURRENT_APP_VERSION = 'v2.9.4';
...
function saveSavingsGoalsToStorage() {
  ...
  if (typeof saveState === 'function') {
    try {
      ...
      console.error('Error triggering saveState from savings goals:', err);
    }
  }
}
window.saveSavingsGoalsToStorage = saveSavingsGoalsToStorage;
...
function openGoalModal(goalId = null) {
  ...
  if (goalId) {
    const targetId = String(goalId);
    const goal = (Array.isArray(savingsGoals) ? savingsGoals : []).find(g => String(g.id) === targetId);
    if (goal) {
      if (titleEl) titleEl.textContent = 'Edit Savings Goal';
      if (editIdEl) editIdEl.value = goal.id;
      if (titleInput) titleInput.value = goal.title || goal.name || '';
      if (targetInput) targetInput.value = goal.targetAmount || '';
      if (savedInput) savedInput.value = goal.savedAmount ?? goal.currentAmount ?? 0;
      if (dateInput) dateInput.value = goal.targetDate || goal.deadline || '';
...
function handleGoalSubmit(e) {
  ...
  const finalSaved = Math.max(0, isNaN(savedAmount) ? 0 : savedAmount);

  if (editId) {
    const editIdStr = String(editId);
    const index = savingsGoals.findIndex(g => String(g.id) === editIdStr);
    if (index !== -1) {
      savingsGoals[index] = {
        ...savingsGoals[index],
        title,
...
function deleteGoal(goalId) {
  ...
  const targetId = String(goalId);
  savingsGoals = (Array.isArray(savingsGoals) ? savingsGoals : []).filter(g => String(g.id) !== targetId);
  window.savingsGoals = savingsGoals;
  saveSavingsGoalsToStorage();
  renderSavingsGoals();
}
...
function openGoalDepositModal(goalId, actionType = 'deposit') {
  ...
  const infoEl = document.getElementById('deposit-modal-info');
  ...
  const submitBtn = document.getElementById('btn-submit-goal-deposit');

  if (goalIdInput) goalIdInput.value = goal.id;
  if (actionTypeInput) actionTypeInput.value = actionType;
  if (amountInput) amountInput.value = '';

  const curSaved = Number(goal.savedAmount ?? goal.currentAmount ?? 0);
  const curTarget = Number(goal.targetAmount || 0);

  if (actionType === 'deposit') {
    if (titleEl) titleEl.innerHTML = `<i class="fa-solid fa-plus-circle text-emerald"></i> Deposit to ${goal.icon || '🎯'} ${escapeHTML(goal.title || goal.name)}`;
    ...
    if (titleEl) titleEl.innerHTML = `<i class="fa-solid fa-minus-circle text-rose"></i> Withdraw from ${goal.icon || '🎯'} ${escapeHTML(goal.title || goal.name)}`;
  ...
  if (infoEl) {
    infoEl.innerHTML = `Current Balance: <strong>${formatCurrency(curSaved)}</strong> / Target: <strong>${formatCurrency(curTarget)}</strong>`;
  }
...
function handleGoalDepositSubmit(e) {
  ...
  let curSaved = Number(goal.savedAmount ?? goal.currentAmount ?? 0);

  if (actionType === 'deposit') {
    curSaved += amount;
    goal.savedAmount = curSaved;
    goal.currentAmount = curSaved;
    if (shouldDeduct) {
      const newExp = {
        id: 'exp_savings_' + Date.now(),
        amount: amount,
        category: 'Services & Subscriptions',
        description: `Savings Allocation (${goal.icon || '🎯'} ${goal.title || goal.name})`,
        date: todayStr,
        createdAt: new Date().toISOString()
      };
    ...
    if (amount > curSaved) {
      alert(`Cannot withdraw more than current balance (${formatCurrency(curSaved)}).`);
      return;
    }
    curSaved -= amount;
    goal.savedAmount = Math.max(0, curSaved);
    goal.currentAmount = Math.max(0, curSaved);
    if (shouldDeduct) {
      const newInc = {
        id: 'inc_savings_' + Date.now(),
        amount: amount,
        source: `Savings Goal Withdrawal (${goal.icon || '🎯'} ${goal.title || goal.name})`,
        date: todayStr,
        createdAt: new Date().toISOString()
      };
...
function renderSavingsGoals() {
  ...
  const totalSaved = savingsGoals.reduce((sum, g) => sum + Number(g.savedAmount ?? g.currentAmount ?? 0), 0);
  ...
  gridEl.innerHTML = savingsGoals.map(goal => {
    ...
    const deadlineVal = goal.targetDate || goal.deadline;
    if (deadlineVal) {
      const targetDt = new Date(deadlineVal);
    ...
    const safeTitle = escapeHTML(goal.title || goal.name || 'Goal');
    const safeNotes = goal.notes ? `<div class="goal-notes-text">${escapeHTML(goal.notes)}</div>` : '';
    ...
    return `
      <div class="goal-card glass-panel">
        <div>
          ...
          <div class="goal-title-text">${safeTitle}</div>
          ${safeNotes}
