  ...
  try {
    ...
    if (Array.isArray(subscriptions) && subscriptions.length > 0) {
      const subRows = subscriptions.filter(isValidSubscription).map(s => ({
        ...
        due_day: Number(s.dueDay || 1),
        start_month: s.startMonth || s.start_month || '',
    ...
    if (Array.isArray(data.subscriptions)) {
      subscriptions = data.subscriptions.filter(isValidSubscription).map(s => {
        ...
        return {
          ...
          dueDay: Number(s.dueDay || s.due_day || (s.due_date ? new Date(s.due_date).getDate() : 15) || 15),
          startMonth: s.startMonth || s.start_month || '',
          start_month: s.startMonth || s.start_month || '',
